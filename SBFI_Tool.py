    ...
    if config.platform == Platforms.Multicore or config.platform == Platforms.Grid:
        ...
        while remaining_jobs:
            ...
            if joblst.total_len() == 0:
                remaining_jobs = False            
            #Check for job hang
            elif t_queue_not_changed > 500 and how_old_file(os.path.normpath(os.path.join(config.call_dir, 'Analyzer.log'))) > 200:
...
def tweak_config_and_check_termination(config):
    ...
    tlim = (config.genconf.std_workload_time / config.genconf.std_clk_period) if f.time_mode == TimeModes.ClockCycle else config.genconf.std_workload_time
    return(f.time_start > tlim)
    


def RunSBFI(datamodel, config, toolconf):
    #Phase 1. Initialize HDL models for simulation: creates simNodes.xml and SimInitModel.do for each HDLmodel
    if config.initializer_phase:
        InitializeHDLModels(config, toolconf)

    #Phase 2. Profiling: fills the table Profiling and appends related data into table Targets
    if config.profiler_phase and datamodel != None:
        ProfileHdlModels(config, toolconf, datamodel)

    #Phase 3. Cleanup, Compile, Create reference simulation script and checkpoints
    if config.injector_phase:
        ...
        for conf in config.parconf:
            ...
            # e.g. adjust the fault injection intervals 
            Finish_Flag = False
            while not Finish_Flag:
                iteration += 1
                with open(os.path.join(config.call_dir, config.injector.campaign_label + '_log.log'), 'a') as logfile:
                    logfile.write('\n\n\n\n' + datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d_%H-%M-%S') + ' : ' + to_string(config.injector.fault_model[0]) + ' : \n\t\tITERATION ' +  str(iteration))
