...
class DevLayout:
    ...
    def get_Tile_local_coordinates(self, globalX, globalY):
        for slr_id, slr in self.slr_by_layout_index.iteritems():
            if globalX >= slr.TileYmin and globalY <= slr.TileYmax:
...
class VivadoDesignModel:
    ...
    def parse_netlist(self, mode='bels'):
        parser_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'SupportScripts', 'VivadoParseNetlist.do')
    ...
    def load_lut_descriptors(self, LutDescTab, unit_path="", pb=None):
        for i in range(LutDescTab.rownum()):
            ...
            item.init = LutDescTab.getByLabel('INIT', i)
    ...
    def map_luts(self, unit_path="", pb=None):
        ...
        print('Mapped LUTS in dut_scope = \"{0:s}\", pblock = {1:s}: {2:d}'.format(unit_path, pb.to_string(), len(self.LutCellList)))
    ...
    def initialize(self):
        ...
        self.CM.print_stat()
        self.CM.log(os.path.join(self.generatedFilesDir, 'bitlogS7.txt'), True, True, True, True)
