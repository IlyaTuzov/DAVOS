...
def eth_config(selene_dir):  
    ...
    if not ETH_INITIALIZED:
        ...
        script = "grmon -u -uart /dev/ttyUSB8 -c {0:s}/grmon_davos.do".format(selene_dir)
...
def boot_linux(selene_dir):   
    ...
    for i in range(MAX_RETRIES):
        try:
            ...
            grmon_proc = subprocess.Popen(script, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, bufsize=1, shell=True)
...
def test_workload(linux_proc, golden_run=False):
    global grmon_proc
    global grmon_message_buf
    grmon_message_buf.append('')    
    try:     
        linux_proc.sendline('./demo_v6')
        time.sleep(1)
        ...
        checksum_head = re.findall("CHECKSUM_HEAD=([0-9xa-fA-F]+)", output)
        checksum_trail = re.findall("CHECKSUM_TRAIL=([0-9xa-fA-F]+)", output)
    except Exception as e:
        return "Status: {{Testbench_Exception: {0:s} }}".format(str(e))     
        
    msg = "result={0:s}, timeout_msg={1:s}, os_msg={2:s}, checksum_head={3:s}, checksum_trail={4:s}".format(
        str(result), str(timeout_msg), str(os_msg), str(checksum_head), str(checksum_trail))        
    if workload_status == False:
        if terminate_workload():
            return "Status: {{Application_Crash: {0:s} }}".format(msg)
        else:
    ...
    if golden_run:
        ...
        if len(os_msg) > 0:
            ...
            if result[0] == 'pass':
                if checksum_head[0] == '0x9f0c822c' and checksum_trail[0] == '0x9f0c822c':
...
def terminate_workload():
    ...
    try:
        ...
        log('crtl+c exception: {0:s}\n'.format(str(e)))          
    try:        
        linux_proc.sendline('pkill demo_v6 -c')
        time.sleep(0.5)
        recover_status = linux_proc.prompt(timeout=5)
        output = linux_proc.before
        log("pkill demo_v6 -c: {0:s}".format(output))        
...
while True:
    ...
    if data == "1":
        ...
        with open("/home2/tuil/selene_axi4/selene-hardware/selene-soc/selene-xilinx-vcu118/grmon_log.txt", 'a') as logfile:
            logfile.write("\n\n{0:s}: {1:s} : FMODE={2:s}".format(str(time.ctime()), str(grmon_message_buf[-1]), res))
            logfile.flush()
