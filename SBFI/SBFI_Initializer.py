...
def InitializeHDLModels(config, toolconf):
    ...
    injection_unit_paths = sorted(list(set([c.unit_path for c in config.SBFI.injection_scopes])))
    observation_unit_paths = sorted(list(set([c.unit_path for c in config.SBFI.observation_scopes])))
    ...
    for c in config.parconf:
        ...
        if os.path.exists(os.path.join(c.work_dir, toolconf.injnode_list)) and not config.SBFI.clean_run:
            ...
            for i in config.SBFI.injection_scopes:
                print('\n\t{0} : Parsing Injection Scope: {1}'.format(c.label, i))
    ...
    for c in config.parconf:
        if os.path.exists(os.path.join(c.work_dir, toolconf.list_init_file)) and not config.SBFI.clean_run:
            ...
            for i in config.SBFI.observation_scopes:
                cdata = ConfigInitNodes(c.label)
                cdata.export_folder = c.work_dir
                print('\n\t{0} : Parsing Observation Scope: {1}'.format(c.label, i))
                with open('parsescript.do', 'w') as parse_script_file:
                    if c.design_type == 'netlist':
                        runscript = "vsim -c -restore {0} -do \"do find instances -file {1} -recursive {2}/{3} \nquit\n\" > parse_log.txt".format(c.checkpoint,
                                                                                                                                                  f_observ_instances,
                ...
                if c.design_type == 'rtl':
                    for x in cdata.all_nodes:
                        subpath_prefix = x.unit_path[len(i.unit_path):].replace('/', '_')
                        obs_item = ("add list {0} -label {{{1}}} {{{2}}}".format(i.sampling_options, subpath_prefix + x.name, x.name)).replace('[', '(').replace(']', ')')
            ...
            for i in config.SBFI.observation_items:
                if len(i.array_items) > 0:
                    trace = '\n'.join(['add list {0} -label {{{1}[{2}]}} {{{3}[{4}]}}'.format(i.sampling_options, i.label if i.label != "" else i.path, k, i.path, k) for k in i.array_items])
                else:
                    trace = "\nadd list {0} -label {{{1}}} {{{2}}}".format(i.sampling_options, i.label if i.label != "" else i.path, i.path)
                if i.group not in trace_items:
                    trace_items[i.group] = []
                trace_items[i.group].append(trace)
