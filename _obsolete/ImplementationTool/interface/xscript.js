		...
		var fullsource = "./XIStat.xml";
        var minsource =  "./XIStat_min.xml";
		var Theaders = ['Label',        'FREQ',         'POWER_DYN',    'SLICE',        'REG',          'LUT',          'DSP',          'RAMB',         'Progress',     'Iteration',    'Clock',        'Converged',    'Synthesis',                     'Translate',                     'Map',                    'PlaceRoute',               'TimingAnalysis',            'NetlistBuilder',              'Fuse_Compile',                'Simulation_ISIM',              'PowerAnalysis'];
		var TLogDest = ['./Logs/@.log', './Logs/@.log', './Logs/@.log', './Logs/@.log', './Logs/@.log', './Logs/@.log', './Logs/@.log', './Logs/@.log', './Logs/@.log', './Logs/@.log', './Logs/@.log', './Logs/@.log', './@/netgen/log/_synthesis.log', './@/netgen/log/_translate.log', './@/netgen/log/_map.log', './@/netgen/log/_par.log', './@/netgen/log/timing.twr', './@/netgen/par/_timesim.vhd', './@/netgen/log/fuse_log.log', './@/netgen/log/isim_log.log',  './@/netgen/log/xpower_log.log'];        
        var glob_tblbody;
		var proc_tblbody;
		var labels = [];
        var GlobalStatHeaders = ['Phase', 'Progress', 'Time_Taken', 'Report']
		...
		function addRowsFromXml(xmlNodes, tableNode, Headers) {
				...
				for (i=0; i<xmlNodes.length; i++) {
					if( xmlNodes[i].hasAttribute(Headers[0]) ){
						var key = xmlNodes[i].getAttribute(Headers[0]);
						...
						if(crow.length > 0) {	//if row for this configuration exists
                            ...
                            labels[newRow.rowIndex] = xmlNodes[i].getAttribute(Headers[0]);
							for(j=0;j<Headers.length;j++){
