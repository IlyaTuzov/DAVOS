...
class DesignNode:
    def __init__(self, tname, fmode_lbl):
        self.name = tname
        self.fmc = {c : 0 for c in fmode_lbl}
        self.fmc_percentage = {c: 0.0 for c in fmode_lbl}
        self.children = []
    ...
    def append(self, pth, fm):
        ...
        for c in self.children:
            if c.name == cnode:
                ...
                break
        if xnode == None:
            xnode = DesignNode(cnode, self.fmc.keys())
    ...
    def normalize(self, relnode):
        total = sum(relnode.fmc.values())
        for k, v in self.fmc.iteritems():
            self.fmc_percentage[k] = (v * 100.0)/total
        for c in self.children:
            c.normalize(relnode)
    ...
    def to_JSON(self):
        res = '{{\n\"name\": \"{0}\",'.format(self.name) + "".join(['\n\"{0:s}\": \"{1:d}\",'.format(k, v) for k, v in self.fmc.iteritems()])
        res += ",".join(['\n\"{0:s}_p\": \"{1:0.2f}\"'.format(k, v) for k, v in self.fmc_percentage.iteritems()])
    ...
    def to_HTML(self, level=0):
        res = "\n<tr>" + "<td><pre>{0}{1}</td></pre>".format("    |"*level, self.name) + "".join(["\n<td><pre>{0:d}</td></pre>".format(self.fmc[k]) for k in sorted(self.fmc.keys())]) + "".join(["\n<td><pre>{0:0.2f}</td></pre>".format(self.fmc_percentage[k]) for k in sorted(self.fmc_percentage.keys())]) + "</tr>"
        for c in self.children:
...
#os.environ['QUERY_STRING'] = "action=search&model=SeleneMC&eind=&target=&instancetype=LUT&injectioncase=&faultmodel=BitFlip&forcedvalue=&injectiontime=&injectionduration=&observationtime=&profiledvalue=&failuremode=&errorcount=&trapcode=&latencyfaultfailure=&samplesize=&randseed="
...
if not os.path.exists(os.path.join(os.getcwd(), 'cache', signature)):
    ...
    failuremode_labels = [c[0] for c in cursor.fetchall()]
    ...
    for k, v  in fmodes.items():
        statistic_content += '\n\t' + k + '_abs=\"' + str(v) + '\"'
        statistic_content += '\n\t' + k + '_err=\"' + str( '%.2f' % (100*get_error_margin(total, 0.95, v*1.0/total, populationsize)) ) + '\"'
        statistic_content += '\n\t' + k + '=\"' + str('%.2f' % (v*100.0/total)) + '\"'
    statistic_content += ' />'
